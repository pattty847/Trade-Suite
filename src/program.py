...
class Program:
    ...
    def add_tab_bar(self):
        ...
        with dpg.tab_bar(parent=self.parent):
            ...
            with dpg.tab(label='Tab 1'):
                with dpg.child_window() as tag:
                    self.chart = Charts(tag)
    ...
    def show_favorites_window(self):
        ...
        with dpg.window(label="Favorites", id="favorites_window", width=400, height=500, on_close=delete_favorites):
            dpg.add_spacer()
            with dpg.child_window(autosize_x=True, autosize_y=False, height=450, horizontal_scrollbar=True):
    ...
    def add_source_modal(self, sender, app_data, user_data):
        ...
        with dpg.window(modal=True, popup=True, width=-1, height=-1, tag='data_source', on_close=delete_source_modal) as source_modal:
